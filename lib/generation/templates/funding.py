...
class {class_name}(IStrategy):
    ...
    def _merge_funding_data(self, dataframe: pd.DataFrame, funding_df: pd.DataFrame) -> pd.DataFrame:
        if funding_df.empty:
            ...
            return dataframe
        funding = funding_df[["date", "funding_rate", "funding_zscore"]].copy()
        funding["date_available"] = funding["date"] + pd.Timedelta(minutes=self.FUNDING_DELAY_MINUTES)
        dataframe = dataframe.sort_values("date").reset_index(drop=True)
        merged = pd.merge_asof(dataframe, funding[["date_available", "funding_rate", "funding_zscore"]],
...
{regime_detection_block}
    ...
    def populate_entry_trend(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        zscore, threshold, regime = dataframe["funding_zscore"], self.ZSCORE_THRESHOLD, dataframe['regime']
